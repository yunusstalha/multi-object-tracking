...
for time in range(measurements.shape[0]):
    ...
    # for measurement in range(measurements[time].shape[0]):
        # x = measurements[time][measurement,0]
        # y = measurements[time][measurement,1]
        # w = measurements[time][measurement,2]
        # h = measurements[time][measurement,3]
        # x = x - w/2
        # y = y - h/2
        # rect = patches.Rectangle((x,y), w, h, linewidth=2,edgecolor='r', facecolor='none')
        # ax.add_patch(rect)

    num_tracks_at_time = 0
    ...
    for track in tracks:
        for track_id in range(track.shape[0]):
            ...
            #     ellipse = patches.Ellipse(center, width=radii[0]*2, height=radii[1]*2,
            #                                 angle=angle, color='b', alpha=0.2)

                # ax.add_patch(ellipse)
                num_tracks_at_time += 1
    ...
    # fig.savefig("exp2/fig_" + str(time) + ".png")
    num_tracks.append(num_tracks_at_time)
# ax.clear()
plt.plot(num_obj)
plt.plot(num_tracks)
plt.legend(['Number of Objects', 'Number of Tracks']) 
plt.grid()
plt.show()
# plt.savefig("exp2/tracking_ability.png")